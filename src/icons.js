function initIcons() {
    /**
     * @typedef {{viewBox: string, inner: string}} IconData
     */

    /**
     * @typedef {Record<string, IconData>} IconMap
     */

    /** @type {IconMap} */
    let ICONS;

    /**
     * @param {string} name
     * @param {string[]} extraClasses
     * @returns {SVGSVGElement | null}
     */
    function createIconSvg(name, extraClasses = []) {
        let icon = ICONS[name];
        if (!icon) {
            console.warn(`Unknown icon: ${name}`);
            return null;
        }
        let svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
        svg.setAttribute("class", ["inline-svg", ...extraClasses].join(" "));
        svg.setAttribute("xmlns", "http://www.w3.org/2000/svg");
        svg.setAttribute("viewBox", icon.viewBox);
        svg.innerHTML = icon.inner;
        return svg;
    }

    /**
     * @param {HTMLElement} elem
     * @returns {string | null}
     */
    function iconNameFromElement(elem) {
        for (let cls of elem.classList) {
            if (cls.startsWith("icon-")) {return cls.slice(5);}
        }
        return null;
    }

    /**
     * @param {ParentNode} root
     * @returns {void}
     */
    function mountIcons(root = document) {
        let targets = root.querySelectorAll("i.icon");
        for (let trg of targets) {
            let iconName = iconNameFromElement(trg);
            if (!iconName) {continue;}

            let extraClasses = [...trg.classList]
                .filter((cls) => cls != "icon" && !cls.startsWith("icon-"));
            let svg = createIconSvg(iconName, extraClasses);
            if (svg) { // replace icon element with svg
                trg.replaceWith(svg);
            }
        }
    }

    ICONS = {
        "code": {
            viewBox: "0 0 24 24",
            inner: `<path stroke="currentColor" stroke-width="2" d="M7 8L3 11.6923L7 16M17 8L21 11.6923L17 16M14 4L10 20"/>`
        },
        "git": {
            viewBox: "0 1 24 24",
            inner: `<path fill-rule="evenodd" clip-rule="evenodd" fill="currentColor" d="M13.4142 3.82843C12.6332 3.04738 11.3668 3.04738 10.5858 3.82843L9.91421 4.5L11.482 6.06774C11.6472 6.02356 11.8208 6 12 6C13.1046 6 14 6.89543 14 8C14 8.17916 13.9764 8.35282 13.9323 8.51804L15.982 10.5677C16.1472 10.5236 16.3208 10.5 16.5 10.5C17.6046 10.5 18.5 11.3954 18.5 12.5C18.5 13.6046 17.6046 14.5 16.5 14.5C15.3954 14.5 14.5 13.6046 14.5 12.5C14.5 12.3208 14.5236 12.1472 14.5677 11.982L13 10.4142V15.2676C13.5978 15.6134 14 16.2597 14 17C14 18.1046 13.1046 19 12 19C10.8954 19 10 18.1046 10 17C10 16.2597 10.4022 15.6134 11 15.2676V9.73244C10.4022 9.38663 10 8.74028 10 8C10 7.82084 10.0236 7.64718 10.0677 7.48196L8.5 5.91421L3.82843 10.5858C3.04738 11.3668 3.04738 12.6332 3.82843 13.4142L10.5858 20.1716C11.3668 20.9526 12.6332 20.9526 13.4142 20.1716L20.1716 13.4142C20.9526 12.6332 20.9526 11.3668 20.1716 10.5858L13.4142 3.82843ZM9.17157 2.41421C10.7337 0.852115 13.2663 0.852119 14.8284 2.41422L21.5858 9.17157C23.1479 10.7337 23.1479 13.2663 21.5858 14.8284L14.8284 21.5858C13.2663 23.1479 10.7337 23.1479 9.17157 21.5858L2.41421 14.8284C0.852115 13.2663 0.852119 10.7337 2.41422 9.17157L9.17157 2.41421Z"/>`
        },
        "link": {
            viewBox: "-10 0 120 120",
            inner:
`<g transform="rotate(-45 50 50)" fill="none" stroke="currentColor" stroke-width="12" stroke-linecap="butt" stroke-linejoin="round">
    <path d="M 38 30 L 15 30 A 20 20 0 0 0 15 70 L 40 70 A 20 20 0 0 0 59.3 45" />
    <circle cx="59.3" cy="45" r="6" fill="currentColor" stroke="none" />
    <path d="M 62 70 L 85 70 A 20 20 0 0 0 85 30 L 60 30 A 20 20 0 0 0 40.7 55" />
    <circle cx="40.7" cy="55" r="6" fill="currentColor" stroke="none" />
</g>`
        },
        "se-webring": {
            viewBox: "0 0 960 960",
            inner: `<path fill-rule="evenodd" clip-rule="evenodd" fill="currentColor" d="M390.499 272.146V687.477C491.737 672.742 569.501 585.38 569.501 479.811C569.501 374.243 491.737 286.881 390.499 272.146ZM367.396 749.527C364.991 749.591 362.577 749.623 360.157 749.623C211.506 749.623 91 628.824 91 479.811C91 330.799 211.506 210 360.157 210C362.577 210 364.991 210.032 367.396 210.096C368.31 210.032 369.233 210 370.163 210H869V749.623H370.163C369.233 749.623 368.31 749.591 367.396 749.527ZM330.687 687.602V272.021C229.023 286.379 150.813 373.94 150.813 479.811C150.813 585.683 229.023 673.244 330.687 687.602ZM529.346 269.958H809.187V689.665H529.346C590.325 640.201 629.313 564.57 629.313 479.811C629.313 395.053 590.325 319.422 529.346 269.958Z"/>`
        }
    };

    mountIcons();
}